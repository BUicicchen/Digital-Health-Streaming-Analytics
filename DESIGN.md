...
The data generator is built based on timestamps. It will switch activities every ten timestamps and there are in total 4 sections of activities, Sleeping, Walking, Eating, and Running. Each activity has a different range for various fields. For details, please check the figure below. Different data ranges are designed based on thorough research.. For example, the diastolic blood pressure(DBP) remains the same for walking, eating, and running. This is because only the systolic blood pressure(SBP) will change according to the activities. DBP will only decrease when heart beat rate decrease to a certain level which is while sleeping. 

There are two types of distribution type that user can choose from while generating the data, uniform distribution or exponential distribution. Uniform distribution will generate numbers randomly from the range. Exponential distribution will only generate a few high values with a large number of low values. The current lambda we use is 0.5. The way we implement the exponential distributions is we have a function to calculate the probability of a number to be picked by the specified lambda and lower bound. Then we generate a random double. If the double is lower than the probability we say this number is picked. In order to avoid the case where the first number in data range is always picked, we start by shuffle the list of number in range. 
...
The data generator for multiple devices can sends data in required format correctly and be configured with (1) different input distributions, such as in order and out of order, inuniform and exponential and (2) different frequencies of data emission.
